...
three{%end%}
    ...
    def test_error_line_number_include(self):
        ...
        try:
            ...
            self.assertTrue("# sub.html:1 (via util.html:1)" in
                            traceback.format_exc())

    def test_error_line_number_extends_base_error(self):
        loader = DictLoader({
            "util.html": "{{1/0}}",
            "sub.html": "{% extends 'util.html' %}",
        })
        try:
...
{% end %}
        ...
        try:
            ...
            self.assertTrue("# sub.html:4 (via util.html:1)" in
                            traceback.format_exc())
...
{% block name %}util: {{ name }}{% end %}""",
            "unescaped_block.html": """\
{% autoescape None %}\
{% block name %}util: {{ name }}{% end %}""",

            # Extend a util template with different autoescape policy,
            # with and without overriding the util's blocks
