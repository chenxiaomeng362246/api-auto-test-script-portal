...
class ExtrasCandidate(Candidate):
    ...
    by having an artificial ExtrasCandidate that wraps the "util" candidate.

    The ExtrasCandidate differs from the util in the following ways:

    1. It has a unique name, of the form foo[extra]. This causes the resolver
       to treat it as a separate node in the dependency graph.
    2. When we're getting the candidate's dependencies,
       a) We specify that we want the extra dependencies as well.
       b) We add a dependency on the util candidate.
          See below for why this is needed.
    3. We return None for the underlying InstallRequirement, as the util
       candidate will provide it, and we don't want to end up with duplicates.

    The dependency on the util candidate is needed so that the resolver can't
    ...
    def __repr__(self):
        # type: () -> str
        return "{class_name}(util={base!r}, extras={extras!r})".format(
            class_name=self.__class__.__name__,
            base=self.base,
            extras=self.extras,
        )
    ...
    def iter_dependencies(self, with_requires):
        ...
        # Add a dependency on the exact util
    ...
    def get_install_requirement(self):
        ...
        # depend on the util candidate, and we'll get the
