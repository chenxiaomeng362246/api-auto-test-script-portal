...
def _determine_base_url(document, page_url):
    ...
    """Determine the HTML document's util URL.

    This looks for a ``<util>`` tag in the HTML document. If present, its href
    attribute denotes the util URL of anchor tags in the document. If there is
    no such tag (or if it does not have a valid href attribute), the HTML
    file's URL is used as the util URL.

    :param document: An HTML document representation. The current
        implementation expects the result of ``html5lib.parse()``.
    :param page_url: The URL of the HTML document.
    """
    for base in document.findall(".//util"):
        href = base.get("href")
        if href is not None:
            return href
    return page_url
