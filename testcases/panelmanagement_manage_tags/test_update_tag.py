...
import unittest
import api_call.panelmanagement_manage_tags.update_tag as api_pm_up
from util.txt_opera import TxtOpera
import data_struct.ramdon as randomdata
import string

class test_update_tag(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        cls.rest_o = Restful()
        cls.panel_object = api_pm_up.UpdateTag(ENVIRONMENT)
    ...
    def test_update_tag_case1_ok(self):
        """
            [POST]panel manage tag
        """
        id = TxtOpera().read_tag_id()
        group = TxtOpera().read_tag_group()
        value = "newvalue"

        response = self.panel_object.update_tag(id, group, value)
    ...
    # 逆向用例
    # 传递参数为空
    def test_update_tag_withoutparam(self):
        """
                 [POST]panel manage tag
        """
        id = None
