    ...
    def test_resource_search_collection_rating(self):
       ...
       4.4.00 [POST]search 资源 平均分 排序
        ...
        for s in rating["items"]:
            if round(s.get("custom_properties").get("avg_rating"),2) >= 1 and glb.rating == 1:
                pass
            elif round(s.get("custom_properties").get("avg_rating"),2) >= 2 and glb.rating == 2:
                pass
            elif round(s.get("custom_properties").get("avg_rating"),2) >= 3 and glb.rating == 3:
                pass
            elif round(s.get("custom_properties").get("avg_rating"),2)>= 4 and glb.rating == 4:
                pass
            elif round(s.get("custom_properties").get("avg_rating"),2) >= 5 and glb.rating == 5:
                pass
            else:
